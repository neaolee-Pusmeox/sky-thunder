...
> 最后更新: 2025-07-17 · v1.0 正式版 · 文件: `Raiden/raiden-game.html` (~3500行, 纯 Canvas 2D + Web Audio API, 零依赖)
...
### 修复 (8项)
- 🐛 **致命**: 按住炸弹键瞬间耗尽 → 加入一键触发保护
- 🧹 清除 5 处重复代码（combo变量/倍率文字/渐变条/Lv4光环）
- 🧹 移除 Lv3 排气粒子冗余计算
- 🧹 移除 enemy.hit() 中多余的 comboMultiplier 赋值

### 新增 (7项)
- ⚡ G 键作弊模式：无敌 + 无限炸弹 + HUD指示器 + 金色旋转光环
- 🔒 作弊模式仅 PLAYING 状态可用，重开自动重置

### 验证
- ✅ JS 语法检查通过
- ✅ 代码审查通过
- ✅ 浏览器端到端测试通过（无 console 错误）

### 状态
**v1.0 正式版** · 87个问题全部修复 · 可游玩全部5关 · 下一个目标: v1.1 品质打磨
